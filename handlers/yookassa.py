...
class YooKassaManager:
    ...
    async def create_payment(self, amount: float, description: str, user_email: str = None, user_id: str = None, tariff_name: str = None, username: str = None, name_account: str = None) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            if name_account:
                metadata["name_account"] = name_account
            
            # Добавляем информацию о пользователе в описание платежа
            user_info = f"(ID: {user_id}, Username: {username or 'Нет'}, Имя: {name_account or 'Нет'})"
            
            if "Пополнение баланса" in description:
                metadata["balance_payment"] = "true"
                description = f"{description} {user_info}"
            elif tariff_name:
            ...
            payment = Payment.create({
                ...
                "confirmation": {
                    "type": "redirect",
                    "return_url": "https://t.me/TOR_VPNbot"
                },
                "capture": True,
